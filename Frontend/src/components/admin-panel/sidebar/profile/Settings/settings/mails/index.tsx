...
import { CheckIcon, MailsIcon, MessageSquareIcon, PencilIcon, Settings } from 'lucide-react';
import Notification from '../UI/Notification'; // Adjust path as needed
import { SettingsHeader } from '../UI/SettingsHeader';

type MailsSettingsForm = {
  notMessage: string;
  billMessage: string;
...
export default function MailsSettings() {
  const { control, handleSubmit, reset, formState: { isDirty, isSubmitSuccessful }  } = useForm<MailsSettingsForm>({
    defaultValues: {
  ...
  const onSubmit = (data: MailsSettingsForm) => {
    console.log(data);
      ...
      <h1 className="text-lg font-bold">SET AGENCY EMAILS</h1>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-3">{Object.entries(control._defaultValues).map(([key, value]) => (
          <div key={key} className="mb-4">
            ...
            <Controller
              name={key as keyof MailsSettingsForm}
              control={control}
              render={({ field }) => (
                <textarea {...field} rows={3} className="mt-1 block text-sm w-full p-2 border border-gray-300 rounded-md"></textarea>
              )}
            />
          </div>
        ))}</div> 
