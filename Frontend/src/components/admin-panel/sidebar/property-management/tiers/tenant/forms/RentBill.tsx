...
const RentBill = () => {
  ...
  const form = useForm<RentFormData>({
    ...
    defaultValues: {
      bills: [{ rent: 0, charge: 0, total: 0, month: "" }],
    },
  });
  ...
  // Function to handle adding new bill
  const handleAddBill = () => {
    append({ rent: 0, charge: 0, total: 0, month: "" });
  };

  const handleFieldChange = (index: number, field: keyof RentFormData['bills'][0], value: string | number) => {
    const parsedValue = typeof value === "string" ? parseFloat(value) : value;
  
    // Dynamically setting the value using the field name
    form.setValue(`bills.${index}.${field}`, parsedValue);
  
    // Getting rent and charge values
    const rent = form.getValues(`bills.${index}.rent`);
    const charge = form.getValues(`bills.${index}.charge`);
  
    // Calculating the total
    const total = (rent || 0) + (charge || 0);
  
    // Dynamically setting the total value
    form.setValue(`bills.${index}.total`, total);
  };
  ...
  useEffect(() => {
    if (contract?.rent_locative) {
      ...
      // Loop through the fields and set values for each bill
      fields.forEach((_, index) => {
        // Set rent and charge values in the form for each bill
        form.setValue(`bills.${index}.rent`, rent);
        form.setValue(`bills.${index}.charge`, charge);
  
        // Calculate total as sum of rent and charge for each bill
        const total = rent + charge;
        form.setValue(`bills.${index}.total`, total); // Set total value for the current bill
      });
    }
...
<FormField
  control={form.control}
  name={`bills.${index}.rent`} // Dynamically access the rent field in the array
  render={({ field }) => (
    <FormItem>
      ...
      <FormControl>
        <Input
          {...field}
          value={form.getValues(`bills.${index}.rent`)} // Set the value from the form state
...
<FormField
  control={form.control}
  name={`bills.${index}.charge`} // Dynamically access the charge field in the array
  render={({ field }) => (
    <FormItem>
      ...
      <FormControl>
        <Input
          {...field}
          value={form.getValues(`bills.${index}.charge`)} // Set the value from the form state
          disabled // Disable the field to make it read-only
          type="number"
          placeholder="Charge"
        />
